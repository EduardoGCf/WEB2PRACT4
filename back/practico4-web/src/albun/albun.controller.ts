...
export class AlbunController {
    ...
    async findAll(): Promise<Partial<Albun>[]> {
        ...
        return albunes.map(albun => ({
            ...albun,
            imagen: `http://localhost:3000/uploads/${albun.imagen}`, // URL completa
        }));
    }
    ...
    async findCancionesByAlbum(@Param("id") id: number) {
        ...
        return canciones.map(cancion => ({
            ...
            imagen: `http://127.0.0.1:3000/uploads/canciones/${cancion.imagen}`, // URL completa de la imagen
            cancion_mp3: `http://127.0.0.1:3000/uploads/${cancion.cancion_mp3}`, // URL completa del MP3
        }));
    }
