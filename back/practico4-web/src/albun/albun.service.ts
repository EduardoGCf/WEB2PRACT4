...
export class AlbunService {
  ...
  async findByName(query: string) {
    return this.albunRepository.find({
      ...
      relations: ["artista"], // Incluir la relación con artista
    });
  }
  ...
  async create(createAlbunDto: CreateAlbunDto): Promise<Albun> {
    ...
    if (!artista) {
      throw new NotFoundException(
        `Artista con ID ${createAlbunDto.id_artista} no encontrado`
      );
    }
  ...
  async update(id: number, updateAlbunDto: UpdateAlbunDto): Promise<Albun> {
    ...
    if (updateAlbunDto.id_artista) {
      ...
      if (!artista) {
        throw new NotFoundException(
          `Artista con ID ${updateAlbunDto.id_artista} no encontrado`
        );
      }

      albun.artista = artista; // Asigna el nuevo artista al álbum
    }
